...
({
  ...
  ({ "grassyfield", COSTUME_NORMAL,
    ({
      ...
      "that edge.",
      0,
      (: TO->from_terrain(TERRAINTUTOR "grassystep") :),
  ...
  ({ "desert", COSTUME_DESERT,
    ({
      ...
      "1000, 0, 900000) is in both the sanddunes and the sandoasis "
      ...
      (: TO->from_terrain(TERRAINTUTOR "sandtent") :),
  ...
  ({ "mountaintop", COSTUME_COLD,
    ({ "rub hands",
      ...
      (: TO->from_terrain(TERRAINTUTOR "mountaincabin") :),
  ...
  ({ "grassyfield", COSTUME_NORMAL,
    ({ "laugh",
      "'In spite of dragging you out here like this, Shaydz hasn't actually "
...
void setup() {
    ...
    hq = find_object(TERRAINTUTOR "foyer");
    impatience = 0;
    lesson_step = 0;
    speeching = 0;
} /* setup() */
...
void come( object where ) {
    ...
    if( where != environment())
        move( where, "$N arrives.", "$N stomps away." );
...
protected void to_terrain() {
    move( TERRAIN_CONTROL, "$N appears.",
      "$N snaps his fingers and vanishes.");
    tell_object( pupil, "Something snags you through space.\n");
    pupil->move_with_look( TERRAIN_CONTROL, "$N appear$s.",
        "$N look$s surprised and disappears.");
} /* to_terrain() */

protected void from_terrain( string to ) {
    move( to, "$N appears.", "$N snaps his fingers and vanishes.");
    tell_object( pupil, "Something snags you through space.\n");
    pupil->move_with_look( to, "$N appear$s.",
        "$N look$s surprised and disappears.");
} /* from_terrain() */
...
protected int modesty() {
    ...
    num = sizeof( filter( INV( environment() ),
       (: living($1) && $1->query_gender() != 1 :) ) );

    if( num ) {
        init_command("'"+( num > 1 ? "Ladies" : "Ma'am" )+", would you "
            "please turn around for a moment?", 1 );
        return 10;
    }
