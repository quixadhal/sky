...
void setup() {
    ...
    set_long("You are in an uninitialized search room.  This is an example "
        "of a room that gets cloned and configured afterwards.  In this "
        "room, it's the function set_marker that's used to define its look, "
        "and most of the exits as well.  And the function set_destination "
        "that is used to find, possibly clone and configure, the rooms "
        "that fit the keywords you search for.  The handler called SEARCH "
        "in the code, is \""+SEARCH +"\" and the one called QUIT_HANDLER, "
        "is \""+QUIT_HANDLER +"\".\n" );
    ...
    add_property("commented functions",
        ({ "set_destination", "query_quit_handler", "query_cloning_info" }) );
...
void set_marker( string word ) {
   ...
   foreach( exit_room in (string *)SEARCH->query_rooms_with_function( word ) ) {
      exit = sprintf( "%s: %s",
                     exit_letter,
                     (string)MAP_H->query_short( exit_room ) );
