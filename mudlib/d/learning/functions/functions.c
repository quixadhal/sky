...
void setup() {
    ...
    set_long("This is the room that might lead you to better understanding "
        ...
        "Try \"list\" to get a list of the functions known by the room, take "
        "the exit named by the function to get to a room with exits to the "
        "examples.\n"
        "If you have something you'd like to see here, please mail "+
        CURRENT_LORD+" and suggest it.\n" );
    ...
    foreach( string word in functions ) {
       add_exit( word, "??", "path" );
...
int do_list() {
    add_succeeded_mess( ({ sprintf( "The following functions have examples "
