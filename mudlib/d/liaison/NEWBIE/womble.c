...
int event_enter(object obj, string mess, object from) {
...
void brooch(mixed arg) {
  object item, tp;
  tp = arg[0];
  if(LIBRARY->query_quest_done(tp->query_name(), "womble's brooch")) {
    do_command("'Thank you for returning my brooch, but you've done "
      "this quest already.");
  } else {
    LIBRARY->set_quest((string)tp->query_name(), "womble's brooch");
