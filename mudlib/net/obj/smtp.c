...
/*
 * $Locker:  $
 * $Id: smtp.c,v 1.3 2000/02/18 17:21:39 turrican Exp $
 * $Log: smtp.c,v $
...
void eventSendMail(mixed to, string from, string mess) {
  ...
  if (stringp(to)) {
    data += ({ ({ "MAIL FROM:<" + from + ".discworld>\r\n",
                  "RCPT TO:<" + to + ">\r\n",
                  "DATA\r\n" }) });
  } else if (arrayp(to) && sizeof(to)) {
      string *tmpdata;
  
      tmpdata = ({ "MAIL FROM:<" + from + ".discworld>\r\n" });
...
protected void eventSuccess() {
  if (sizeof(data) && sizeof(data[0])) {
    TPC(sprintf("SMTP: Writing %s\n", data[0][0]));
    eventWrite(data[0][0]);
    data[0] = data[0][1..];
  } else if (sizeof(data) && !sizeof(data[0])) {
    TPC("SMTP: No more data, next message.\n");
    data = data[1..];
    if (!sizeof(data)) {
      TPC(sprintf("SMTP: Writing QUIT\n"));
      eventWrite("QUIT\r\n");
    } else
      eventSuccess();
  }
...
protected void eventForward(string who) {
  if (who && who != "") {
    TPC(sprintf("SMTP: Forwarding to %s.\n", who));
    eventWrite("RSET\r\n");
    eventWrite(sprintf("RCPT TO:<%s>\r\n", who));
  } else {
    TPC("SMTP: Told to forward, but no forwardee...\n");
    eventWrite("QUIT\r\n");
  }
}
...
protected void eventRead(int fd, string mess) {
  ...
  switch (mess[0..2]) {
    ...
    case "553":
    case "554":
      TPC(sprintf("<FATAL> SMTP: %s", mess));
      eventWrite("QUIT\r\n");
      break;
    case "551":
      if (sscanf(mess, "%*s<%s>%*s", forward) != 3) {
        TPC(sprintf("SMTP: %s", mess));
        break;
      }
