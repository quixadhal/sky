...
int calc_new(object who, string board) {
  ...
  news_rc = BOARD_H->query_newsrc(who->query_name());
  if ( !news_rc )
    news_rc = ([ ]);

  stuff = (mixed *)BOARD_H->get_subjects(board);
  if ( !sizeof( stuff ) )
    return 0;
...
int do_storeall(string file) {
  ...
  boards = (string *)BOARD_H->query_boards();
  ...
  for (i=0;i<sizeof(boards);i++) {
    reset_eval_cost();
    if (!BOARD_H->query_killfile(this_player()->query_name(), lower_case(boards[i]))) {
      stuff = (mixed *)BOARD_H->get_subjects(boards[i]);
      if (!sizeof(stuff))
        continue;
      ...
      if (amt != sizeof(stuff)-1) {
       ...
       stuff[j][B_SUBJECT][0..(int)this_player()->
                                                 query_cols() - 10]+"\n\n"+
                         BOARD_H->get_message(boards[i], j)+"\n\n\n");
...
int do_store(int which, string file) {
  ...
  if (!write_file(file,stuff[which][B_SUBJECT]+"\n\n"+
                  BOARD_H->get_message(board_name, which))) {
...
int do_next()
{
  ...
  if ((i = member_array(board_name, boards)) == -1)
    ...
    i = (i+1)%sizeof(boards);
  board_name = boards[i];
  write("Changed to board "+CAP(board_name)+".\n");
  return 1;
} /* next() */
...
int do_killfile(string arg) {
    ...
    foreach ( board in bits ) {
        if ( sizeof( BOARD_H->get_subjects( board ) ) ) {
          BOARD_H->set_killfile(this_player()->query_name(),
                lower_case( board ));
...
int do_unkillfile(string arg) {
  if (!BOARD_H->query_killfile(this_player()->query_name(),
                                  lower_case( arg ) )) {
     add_failed_mess("Board "+arg+" is not kill file anyway.\n");
     return 0;
  } else {
    BOARD_H->remove_killfile(this_player()->query_name(),
                                lower_case(arg));
...
int do_listkillfile() {
   ...
   boards = BOARD_H->list_killfile(this_player()->query_name());
   if ( sizeof( boards ) )
...
int do_board(string str) {
  ...
  if( !BOARD_H->test_can_read( str, TO, TP->query_name() ) ) {
      add_failed_mess("You don't have access to "+str+", sorry.\n");
      return 0;
  }

  write("Okay, changed to board "+ str);
  i = calc_new( TP, boards[i] );
  if(i)
...
int do_boards() {
   write( "The current boards are:\n\""+
         implode( filter( (string *)BOARD_H->query_boards(),
             (: BOARD_H->test_can_read( $1, TO, TP->query_name() ) :) ),
             "\", \"" ) + "\".\n" );
   return 1;
} /* boards() */
...
int do_security() {
   write( "The current people in this boards security array are:\n"+
        implode((string *)BOARD_H->query_security(board_name), ", ")+"\n");
  return 1;
} /* do_security() */
...
int do_add(string name) {
  if (this_player() != this_player(1) ||
    ...
    return 0;
  return (int)BOARD_H->add_allowed(board_name, name);
} /* do_add() */
...
int do_remove(string name) {
  if (this_player() != this_player(1) ||
      !interactive(previous_object()))
    return 0;
  return (int)BOARD_H->remove_allowed(board_name, name);
} /* do_remove() */
...
int do_timeout(int i) {
  if (this_player() != this_player(1) ||
      !interactive(previous_object()))
    return 0;
  return (int)BOARD_H->set_timeout(board_name, i);
} /* do_timeout() */
...
int do_minimum(string i) {
  if (this_player() != this_player(1) ||
      !interactive(previous_object()))
    return 0;
  return (int)BOARD_H->set_minimum(board_name, i);
} /* do_minimum() */
...
int do_maximum(int i) {
  if (this_player() != this_player(1) ||
      !interactive(previous_object()))
    return 0;
  return (int)BOARD_H->set_maximum(board_name, i);
} /* do_maximum() */
...
int do_archive(string name) {
  if (!name)
    return 0;
  if (name != "none")
    return (int)BOARD_H->set_archive(board_name, name);
  return (int)BOARD_H->set_archive(board_name);
} /* do_archive() */
...
int do_skip(string str, int move_on) {
  ...
  news_rc = BOARD_H->query_newsrc(this_player()->query_name());
   if ( !news_rc )
      news_rc = ([ ]);
  otherstuff = (mixed *)BOARD_H->get_subjects(str);
...
int do_arts() {
  ...
  otherstuff = (mixed *)BOARD_H->get_subjects(board_name);
...
int do_transfer(int num, string str) {
  ...
  mess = BOARD_H->get_message(board_name, num-1);
  if (mess && BOARD_H->add_message(str, otherstuff[num-1][B_NAME],
