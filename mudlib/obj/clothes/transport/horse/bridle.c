...
void setup(){
  ...
  set_name("bridle" );
  set_short("snaffle bit leather bridle" );
  add_adjective(({ "snaffle", "bit", "leather" }) );
  set_type("headband" );
  set_weight(5 );
  set_worn_by_race("horse" );
  set_direction_difficulty_modifier(65 );
  set_long("Straps down the side which run up over the "
           ...
           "The snaffle bit is two large rings connected by a bar.\n");
} /* setup() */
