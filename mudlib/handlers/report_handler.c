...
string query_new_cases() {
  ...
  for(i = 0; i < sizeof(new_cases); i++) {
    ...
    reports += "%^CYAN%^Player%^RESET%^: "+capitalize(new_cases[i][R_NAME]);
    for(j = 0; j < (12 - sizeof(new_cases[i][R_NAME])); j++) {
      reports += " ";
    }
...
string query_new_case(int i) {
  ...
  stuff = new_cases[i - 1];
  report += "  %^CYAN%^Suspect%^RESET%^: "+capitalize(stuff[R_NAME])+"\n";
...
string query_recent_cases() {
  ...
  for(i = 0; i < sizeof(recent_cases); i++) {
    ...
    reports += "%^CYAN%^Suspect%^RESET%^: "+capitalize(recent_cases[i][R_NAME]);
    for(j = 0; j < (12 - sizeof(recent_cases[i][R_NAME])); j++) {
      reports += " ";
    }
    ...
    for(j = 0; j < sizeof(check_case); j++) {
      if(arrayp(check_case[j][CHARS])) {
        if(member_array(capitalize(name), check_case[j][CHARS]) != -1) {
          ...
          other_chars += ({check_case[j][R_NAME]});
        }
      }
    }
  }
