...
#ifdef DEBUG
      if(stringp(euid))
        tell_object(find_player("shaydz"), sprintf("Checking perms: %s [%s](string)\n", rcspath, euid ));
      else if(objectp(euid))
        tell_object(find_player("shaydz"), sprintf("Checking perms: %s [%s](object)\n", rcspath, file_name(euid )));
      else
        tell_object(find_player("shaydz"), sprintf("Checking perms: %s [%O](unknown)\n", rcspath, euid ));
#endif
...
#ifdef DEBUG      
          if(objectp(euid))
            tell_object(find_player("shaydz"), sprintf("Write denied to %O or %O Locked by %s\n", euid, previous_object(-1), lockname));
          else
            tell_object(find_player("shaydz"), sprintf("Write denied to %s or %O Locked by %s\n", euid, previous_object(-1), lockname));
#endif
          return 0;
          
          } else {
#ifdef DEBUG      
            tell_object(find_player("shaydz"), "Write allowed to "+geteuid(stack[i])+" Locked by:"+lockname +" on file "+path+"\n");
#endif
...
#ifdef DEBUG
          if(objectp(euid))
            tell_object(find_player("shaydz"), sprintf("Write allowed by %O Locked by %s\n", euid, lockname));
          else
            tell_object(find_player("shaydz"), sprintf("Write allowed by %s Locked by %s\n", euid, lockname));
#endif
        }
      } else {
#ifdef DEBUG
          tell_object(find_player("shaydz"), "No locks found\n");
#endif  
          return 0;
      }
    }
