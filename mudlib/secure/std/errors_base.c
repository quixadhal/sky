 ...
 * <LI>OldDirectory - The direcroy the report belonged to before forwarding
 * </UL>
...
protected varargs string init_errors(mixed key, string user, int replace) {
  ...
  if( ( ret = catch( FD = db_connect("localhost", CONFIG_DB_ERRORS, CONFIG_DB_USER ) ) ) ) {
...
protected mixed get_forwards(mixed key, int row) {
  ...
  err = catch( fd = db_connect("localhost", CONFIG_DB_ERRORS, CONFIG_DB_USER ) );
  ...
  if( err = catch( ret = db_exec( fd, query ) ) ) {
      catch(db_close(fd));
      return err;
  }
...
private string save_status(mixed key, int *ids, string user, string status) {
  ...
  if( sizeof(ids) ) {
      ...
      if( err = catch( res = db_exec( FD, query ) ) )
          ret += err;
      if( stringp(res) )
          ret += res;
  }
...
protected string save_changes(mixed key, string user) {
  ...
  int *ids_fixed, *ids_denied, *ids_fixing, *ids_considering, ftime;
  mixed res, row;
  ...
  ids_fixed = ids_denied = ids_fixing = ids_considering = queries = ({ });
  ret = "";
