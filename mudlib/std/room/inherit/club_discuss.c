...
void init() {
   add_command("discuss",
               ...
               "create account} in <string'club name'>",
 ...
 */
protected int do_discuss_description(int open, string club_name) {
   ...
   TP->do_edit(CLUB_HANDLER->query_description(club_name),
...
private void discuss_finish_description(string str, mixed *junk) {
   ...
   write("Adding the description:\n" + str +
         "\nIs this the correct desciption (yes/no/edit)? ");
...
protected void description_confirm(string confirm, string str, int open, string club_name) {
   ...
   if (strlen(confirm) && confirm[0] == 'e') {
      TP->do_edit(str, (: discuss_finish_description :), 0, 0,
                       ({ open, club_name }));
      return ;
   }

   if (!strlen(confirm) || confirm[0] != 'y') {
      write("Aborting.\n");
      return ;
   }
...
protected void discussion_add_name(string str, int open, string club_name) {
   ...
   write("What message do you wish display as to why you wish this position "
         "added?\n");
   TP->do_edit("", (: discussion_finish_information :), 0, 0,
                   ({ club_name, "Adding the position " + str,
                   CLUB_DISCUSS_NEW_POSITION | open, str, 0 }) );
} /* discussion_add_name() */
...
protected void discussion_remove_member_name(string str, int open, string club_name) {
   ...
   // Check to see if this position is already in the club.
   if (!CLUB_HANDLER->is_member_of(club_name, str)) {
   ...
   TP->do_edit(CLUB_HANDLER->query_description(club_name),
                          (: discussion_finish_information :), 0, 0,
                          ({ club_name, "Removing the member " + str,
                          CLUB_DISCUSS_MEMBER_REMOVE | open, str, 0 }) );
...
protected void remove_position(string str, int open, string club_name) {
   ...
   if (CLUB_HANDLER->is_basic_position(club_name, str)) {
      write("Unable to remove the basic positions from the club.\n");
      return ;
   }

   write("What description do wish to associate with removing the "
         "position " + str + ".\n");
   TP->do_edit("", (: discussion_finish_information :), 0, 0,
                   ({ club_name, "Remove the positon " + str,
                   CLUB_DISCUSS_POSITION_REMOVE | open, str, 0 }) );
} /* remove_position() */
...
protected void change_position_new_name(string str,
   ...
   write("What description to you wish to give for changing the position " +
         position + " to " + str + ".\n");
   TP->do_edit("", (: discussion_finish_information :), 0, 0,
                   ({ club_name, "change the position " + position +
                   " to " + str, CLUB_DISCUSS_POSITION_NAME | open,
                   position, str }) );
...
protected void no_confidence(string str,
   ...
   write("What description do you wish to give for a vote of no confidence "
         "in " + str + "?\n");
   TP->do_edit("", (: discussion_finish_information :), 0, 0,
                   ({ club_name, "no confidence in " + str,
                   CLUB_DISCUSS_NO_CONFIDENCE | open, bing, 0 }) );
} /* no_confidence() */
...
protected void create_account(string str, int open, string club_name) {
   ...
   write("What description do wish to associate with createing the "
         "account " + str + ".\n");
   TP->do_edit("", (: discussion_finish_information :), 0, 0,
                   ({ club_name, "Create the special account " + str,
                   CLUB_DISCUSS_CREATE_ACCOUNT | open, str, 0 }) );
} /* create_account() */
...
protected void discuss_memo_description(class memo_item memo) {
   ...
   TP->do_edit(memo->description, (: discuss_memo_finish_description :),
               0, 0, memo);
...
int do_discuss(int open, string type, string club_name) {
   ...
   if (!CLUB_HANDLER->holds_any_position_of(club_name, TP->query_name())) {
      if (type != "no confidence" &&
          type != "remove member") {
         add_failed_mess("Non elected officals can only add discussion items "
                         "of 'no confidence' and 'remover member'.\n");
         return 0;
      }
   }
