...
/*
 * $Locker:  $
 * $Id: splinted_leg.c,v 1.4 1999/07/20 15:10:52 tannah Exp $
 *
 *
 */
 ...
 * @see help::effects/other
 */

#define THIS_EFF "/std/effects/healing/splinted_leg"
#define BREAK_EFF "/std/effects/other/broken_leg"
#define CLASS     "body.damage.leg.splinted"
#define SHADOW    "/std/shadows/healing/splinted_leg"
...
/** @ignore yes */
mixed beginning( object player, mixed args ) {
  int left_splint;    /* left splint status */
  int right_splint;   /* right splint status */
  string which;       /* which leg is being splinted? */
...
mixed *merge_effect( object player, mixed oldargs, mixed newargs ) {
  ...
  if( action == REMOVE ) {
    ...
    if( which == "left" ) {  /* is my left leg still broken? */
      player->adjust_bonus_dex( 2 );
      if( !oldargs[1] ) 
   ...
   /* thanks to Taffyd for this bit: */
    ...
    if( which == "right" ) { /* is my right leg still broken? */
      player->adjust_bonus_dex( 2 );
      if( !oldargs[3] ) 
  ...
  if( action == HEAL ) {
    ...
    if( which == "left" )  {
      if( !oldargs[1] )
        tell_object( player, "Your left leg feels whole again.\n" );
      oldargs[1] = 1;
    }
    if( which == "right" ) {
      if( !oldargs[3] )
        tell_object( player, "Your right leg feels whole again.\n" );
      oldargs[3] = 1;
    }
    return oldargs;
  }
