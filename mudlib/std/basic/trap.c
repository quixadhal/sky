...
void setup_trap(int difficulty, string trigger, string description,
  ...
  if(trigger == "open")
    TO->set_open_trap(TO, "basic_trap");
  else
    TO->set_lock_trap(TO, "basic_trap");
...
int basic_trap(string action) {
  ...
  switch(TASKER->perform_task(TP, SKILL, _trap_difficulty, TM_FREE)) {
  case AWARD:
  ...
  default:
    if(arrayp(_trap_message)) {
      ...
      say(_trap_message[1], TP);
    } else
      write(_trap_message);

    if(intp(_trap_effect)) {
      if(_trap_effect > TP->query_hp())
        TP->do_death();
      else
        TP->adjust_hp(-(_trap_effect));
    } else if(arrayp(_trap_effect)) {
      if(sizeof(_trap_effect) == 2)
        TP->add_effect(_trap_effect[0], _trap_effect[1]);
      else
        TP->add_effect(_trap_effect[0], _trap_effect[1..]);
    } else
      TP->add_effect(_trap_effect);
    return 0;
  }
}
