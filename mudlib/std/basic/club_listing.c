...
protected object person_in_control(object ob) {
   if (ob && environment(ob) && !ob->query_channel_off()) {
 ...
 * @param online print only the online members
...
protected int club_list(string person,
   ...
   foreach (club in clubs) {
    ...
    if (!found && !check_list) {
       add_failed_mess("No clubs matching the criteria were found.\n");
       return 0;
    }
    if (!found) {
       return 0;
...
protected string club_info_string(string name,
   ...
   if( sizeof(online) )
       ret += "The online members are: "+sort_and_capitalise(online)+".\n";

   ret += "The not currently online members are: "+sort_and_capitalise(
       CLUB_HANDLER->query_members(name) - online )+".\n";

   if( !CLUB_HANDLER->is_family(name) )
       ret += "The recruiters are: "+sort_and_capitalise(
           CLUB_HANDLER->query_recruiters(name) )+".\n";
   ...
   if( CLUB_HANDLER->is_elected_club(name) ) {
       ...
       foreach( pos in positions ) {
           ret += CAP(pos)+": "+CAP(
               CLUB_HANDLER->query_position_members( name, pos ) )+"\n";
      }

      if( CLUB_HANDLER->is_nomination_in_progress(name) && show_election ) {
          ...
          foreach (pos in positions) {
            ...
            if (sizeof(nominations)) {
               ...
               foreach (nom in nominations) {
                  ...
                  ret += "    " + capitalize(nom)+" nominated by "+
                      sort_and_capitalise( CLUB_HANDLER->query_nominators_for(
                      name, pos, nom ) )+last;
               }
           } else {
...
/*
   foreach (per in CLUB_HANDLER->query_members(name)) {
      ...
      wombles = sort_array(wombles,
                           (: strcmp($1->related_to, $2->related_to) :));
      gender = LOGIN_OB->test_gender(per);
      if (gender) {
         gender -= 1;
      }
      if (sizeof(wombles)) {
 ...
 */
   ...
   ret += "The members are: " +
          ...
          ".\n";
   ret += "Refer each member for a detailed list of their "
          "relationships.\n";
   return ret;
