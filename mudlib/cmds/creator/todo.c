...
string query_todo_line(class todo_item item,
                       int cols) {
...
string query_todo_list_string(class todo_item* items) {
   ...
   if (sizeof(items)) {
      foreach (bing in items) {
         data += query_todo_line(bing, this_player()->query_cols()) + "\n";
      }
   }
   return data;
} /* query_todo_list_string() */

private int list_todo() {
   class todo_item* items;
   string data;

   items = query_todo_list(this_player()->query_name());
   if (sizeof(items)) {
      data = query_todo_list_string(items);
   } else {
...
private int complete_item(int id) {
   ...
   if (item) {
      write(query_todo_line(item, this_player()->query_cols()) +
