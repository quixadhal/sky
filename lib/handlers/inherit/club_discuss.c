 ...
 */
inherit "/handlers/inherit/club_elected";
...
private nosave mapping _club_discussion_items;
...
private void add_club_discussion(int type,
                                 function func) {
   _club_discussion_items[type] = func;
} /* add_club_discussion() */
...
string query_discussion_item_short_description(string club_name,
   ...
   if (bing) {
      if (_club_discussion_items[bing->discuss_type & CLUB_DISCUSS_TYPE_MASK]) {
...
int is_valid_discussion_item_type(int type) {
   if (_club_discussion_items[type & CLUB_DISCUSS_TYPE_MASK]) {
      return 1;
   }
   return 0;
...
int complete_discussion_item(string club_name,
   ...
   if (frog &&
      ...
      if (is_discussion_item_quorum(club_name, id)) {
         if (_club_discussion_items[frog->discuss_type & CLUB_DISCUSS_TYPE_MASK]) {
...
protected mixed send_discuss_message(string club_name,
                                     ...
                                     string arg1,
                                     string arg2) {
   return evaluate(_club_discussion_items[item->discuss_type &
