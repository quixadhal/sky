...
void set_stage( int number ) { stage = number; }
...
object make_spore( object place ) {
  ...
  spore->set_delay( delay );
  spore->set_large_weight( large_weight );
  spore->set_weight_per_bite( query_weight_per_bite() );
  spore->set_adjs( adjs );
  spore->set_extra( extra );
  spore->set_type( type );
  spore->set_messages( messages );
  spore->move( place );
  return spore;
} /* make_spore() */
