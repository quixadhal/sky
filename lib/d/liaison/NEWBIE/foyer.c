...
/*
 * $Locker:  $
 * $Id: foyer.c,v 1.11 2000/06/26 16:16:12 wyvyrn Exp $
 */
#include "path.h"
#include <library.h>
#include <config.h>
...
void setup() {
  ...
  add_item( "table", "The table is made of a single piece of green granite, "
           ...
           "like a chessboard.  There seems to be writing scratched into "
           "one square.");
  ...
  add_item("mural", "On one wall there is a painting of a seedy looking inn "
   ...
   add_sign("This is a simple white sign marked \"Discworld\".\n",
            ...
            0, ({ "sign","discworld","Discworld" }) )->add_property(
            "determinate","the ");
  ...
  add_property("no godmother", 1);
   ...
   add_exit("out", CONFIG_START_LOCATION, "path" );
   modify_exit( "out", ({ "function", "check_allow" }) );
} /* setup() */

int check_allow( string str, object ob, string special_mess ) {
  if( LIBRARY->query_quest_done( ob->query_name(), "womble's brooch" ))
      return 1;
