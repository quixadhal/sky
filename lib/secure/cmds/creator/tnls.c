...
int t_file_size(string file) {
  write("File_size: " + file + "\n");
  fsize_count++;
  return file_size(file);
}


mixed *t_get_dir2(string dir) {
  dir_count++;
  write("get_dir2: " + dir + "\n");
  return get_dir(dir);
}
mixed *t_get_dir(string dir, int flag) {
  dir_count++;
  write("get_dir: " + dir + "\n");
...
int ls(string str, int mask) {
  ...
  path = str;
  write("Str: " + str + "\n");
  if (t_file_size(str) == -2 && str[strlen(str)-1] != '/' &&
      !(mask & MASK_D)) {
    path += "/";
  }
  if (mask & MASK_A) {
