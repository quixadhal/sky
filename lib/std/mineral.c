...
#define HANDLER "/handlers/cmr_handler"
...
void init() {
   TP->add_command( "chip", TO, "%D 'with' %I" );
   TP->add_command( "chip", TO, "%D 'using' %I" );
   TP->add_command( "smash", TO, "%D 'with' %I" );
   TP->add_command( "smash", TO, "%D 'using' %I" );
} /* init() */
...
string long( string str, int dark ) {
   ...
   ret = bit1 + (string)HANDLER->identify_material( mineral, TP, 1 )
      + bit2;
   return ret;
} /* long() */
...
string *query_adjectives() {
   string ret;
   ret = HANDLER->identify_material( mineral, TP, 0 );
...
mixed query_static_auto_load() {
   if ( file_name( TO )[ 0 .. 11 ] == "/std/mineral" )
