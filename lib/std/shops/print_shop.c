...
string query_dynamic_auto_load_info(int num);
...
void init() {
    ...
    if( !office_room ) {
        ...
        // removed as it's totally pointless
        ...
        if( sizeof(translation_costs) ) {
            ...
            languages = implode(
                keys(translation_costs) + ({ default_language }), "|");
            add_command("print", "<indirect:object> in {" + languages + "}",
                (: do_print($1, $4[1]) :));
            add_command("print",
                "<number> [copies] of <indirect:object> in {"+languages+"}",
...
protected int do_claim(object *obs) {
    ...
    if (!sizeof(new_copyright)) {
        if (sizeof(cannot_copyright)) {
            add_failed_mess("You cannot copyright "+query_multiple_short(cannot_copyright)+".\n" );
        }
        else if (sizeof(already_copyright) == 1) {
            add_failed_mess(CAP(query_multiple_short(already_copyright))+" is already copyrighted.\n");
        } else {
            add_failed_mess(CAP(query_multiple_short(already_copyright))+" are already copyrighted.\n",);
        }
        return 0;
    }
    ...
    add_succeeded_mess( ({ "", "$N enquires about copyrighting "
        +query_multiple_short(new_copyright)+".\n" }));
    return 1;
} /* do_claim() */
 ...
 * @see /handlers/language->query_language_magic()
 */
...
object find_book_or_paper(object *obs) {
    ...
    if (!sizeof(books) && !sizeof(paper)) {
        if (sizeof(nothing)) {
            add_failed_mess("Unable to print $I as "+((sizeof(nothing)==1)? "it is" : "they are" )+" not a book or paper.\n", nothing);
        }
        return 0;
    }
...
void do_print_run(object ob, int number, int cost, string language) {
    ...
    if (no_collection) {
        write("The copies will be ready at " +
          mudtime(stuff[<1][COLLECT_TIME]) + " for 'collect'ion at the "
          "collection office.\n");
    } else {
        write("The copies will be ready at " +
          mudtime(stuff[<1][COLLECT_TIME]) + " for 'collect'ion.\n");
    }
} /* do_print_run() */
...
void print_pricing(object ob, string language) {
    ...
    write("  1 for " + (string)MONEY_HAND->money_value_string(
      ...
      mudtime(time_to_collect + time_per_copy + time()) + ".\n");;
    for (i = 0; i < sizeof(discount_schedule); i++) {
        ...
        if (num < MAX_NUMBER) {
            write(sprintf("%3d for %s and will be ready at %s.\n", num,
                ...
                mudtime(time_to_collect + time_per_copy * num + time()) ));
        }
    }
 ...
 */
int do_print_new() {
    tell_object(TP,"Edit what you would like your leaflet to say.\n");
    TP->do_edit(0, "finish_write_message");
    return 1;
} /* do_print_new() */
...
void finish_write_message(string input){
    ...
    if(!input || input == ""){
        write("What would be the point of printing a leaflet with no writing on it?\n");
        return;
    }
    ob = clone_object(LEAFLET_OBJ);
    ob->set_name( "leaflet" );
    ob->set_short( "leaflet");
    ob->set_main_plural( "leaflets" );
    ob->set_long("This looks like a leaflet churned out from one of the cities "
      "printing presses.\n");
    ob->set_weight( 0 );
    ob->set_value( 1 );
    ob->add_property("author",TP->query_name());
    ob->set_read_mess( input);

    print_pricing(ob, default_language );

    tell_object(TP,"How many copies would you like? :");
...
void check_number(string str, object ob){
    ...
    if (number < 1) {
        tell_object(TP,"Must print at least one copy.\n");
        return;
    }

    if (number > MAX_NUMBER) {
        tell_object(TP,"Cannot print more than " + MAX_NUMBER +
          " copies.\n");
        tell_object(TP,"How many copies would you like? :");
        input_to("check_number", ob);
        return;
    ...
    cost = price_object(ob, number, default_language);
    ...
    if (player_money < cost) {
        tell_object(TP,"This will cost you " +
          (string)MONEY_HAND->money_value_string(
            cost, place ) + " and you do not have enough "
          "money to afford it.\n");
        return;
    ...
    input_to("check_cost", 0, ob, number, cost, default_language);
