...
/*
 * $Locker:  $
 * $Id: harry_shadow.c,v 1.3 2000/04/26 14:17:23 rhinehold Exp $
...
int set_hp( int number, object thing ) {                     
   ...
   if ( ( number > current ) || !protecting || !thing || !thing->query_property( PKER ) )
...
int attack_by( object thing ) {
   if ( !protecting || !thing->query_property( PKER ) )
