...
/*
 * $Locker:  $
 * $Id: water.c,v 1.6 1999/05/25 23:39:50 pinkfish Exp $
 * $Log: water.c,v $
...
#define SWIMMING_SKILL "other.swimming"
...
void calc_swim_bonus()  {
   ...
   if ( living( swimmer )  &&
      ...
      if ( (int)swimmer->query_burden() < 11 )
         buoyancy = bonus + (int)swimmer->query_property( BUOYANT_PROP );
      else buoyancy = bonus - (int)swimmer->query_max_weight() *
                      ( (int)swimmer->query_burden() - 10 ) / 100 +
