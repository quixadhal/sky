...
/*Thanks to Turrican, Gruper, and Mansarde, without whom I would have given up on this project.
  - Aragorn                                                                                   */
...
int cmd( object player ) {
  ...
  corpses = filter( corpses,
                    (: $1 &&
                       $1->query_ownership() == $( this_player()->query_name() ) :) );

  if( sizeof( corpses ) ) { 
    foreach( corpse in corpses ){
      if ( member_array( player->query_name(), corpse->query_permitted() ) != -1 ){ /*Let's see if any of these corpses have already been permitted*/
        i++;
