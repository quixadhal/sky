...
mixed cmd_ob(object* obs) {
   ...
   foreach (thing in obs) {
      if(living(thing) && thing->a_short(dark)) {
         if(thing == TP) {
            tmp = "Why not just use \"inventory\"?\n";
         }
         else {
            race = thing->query_race();
            holding = thing->query_holding();
            holding = uniq_array(thing->query_holding());
            if( pointerp(holding) ) holding -= ({ 0 });
            tmp = thing->the_short()+( !race && !sizeof(holding) ? "" : " is "+
                   ( race ? "a "+thing->query_gender_string()+" "+thing->query_race() : "" )+
                   ( sizeof(holding) ?( race ? ", " : "" )+
                   "holding "+query_multiple_short(holding) : "" ) )+".\n";                                 tell_object(thing, TP->the_short()+" glances furtively at you.\n");
         }
         tell_object(TP, tmp);
      }
      else {
